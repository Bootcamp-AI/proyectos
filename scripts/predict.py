...
r"""A simple to verify online prediction works.

Example usage:
python predict.py --project my-project --model my-model --version v1
"""
...
def predict(project, model, instances, version=None):
  ...
  response = service.projects().predict(
    name=name, body=instances).execute()
...
def main(_):
  ...
  with open(path) as f:
    ...
    print(predict(FLAGS.project, FLAGS.model, examples, FLAGS.version))
